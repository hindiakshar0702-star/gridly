...
export const renderModularGrid: GridRenderer = (host, options, size) => {
  ...
  const colW = Math.max(0, (containerWidth - totalGutters) / safeCols);
  ...
  if (containerWidth <= 0 || colW <= 0 || rowH <= 0) return;

  for (let r = 0; r < safeRows; r++) {
    for (let c = 0; c < safeCols; c++) {
      // Constraint: x(c) = containerX + c * (colW + gutter)
      const x = containerX + c * (colW + gutter);
